...
3. **候補 URL を開いて検証**: 本文、公開日/更新日、一次ソース性を確認する。公開/更新日が直近 48 時間外、日付不明で速報性が確認できない、検索結果スニペットだけで判断している候補は除外する
4. 各カテゴリで重要度 TOP3 を選定（プロジェクトへの示唆を優先、ステップ 1 で得た投稿済み URL は除外）。URL が別でも、同じ発表・同じ出来事・転載/要約記事など実質的に同一ニュースなら 1 件だけ採用する
