...
3. **キューファイル** `/Users/aquariumy/.claude/data/morning-brief/queue.jsonl` に 12 件を JSONL 形式で書き出す
4. ラッパースクリプトを **1 回だけ** 呼び出して全件処理

# キューファイル形式

`/Users/aquariumy/.claude/data/morning-brief/queue.jsonl` に以下を **既存内容を上書き** で書く。1 行 = 1 ニュース、合計 12 行。順番は A→B→C→D（各カテゴリ内は重要度順）:
...
```bash
/Users/aquariumy/.claude/bin/post-chatwork.sh --process-queue /Users/aquariumy/.claude/data/morning-brief/queue.jsonl 436416910
```
