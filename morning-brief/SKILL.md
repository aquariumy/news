...
4. **キューファイル** `/Users/aquariumy/.morning-brief/queue.jsonl` を **必ず Read してから** Write で上書きする（既存ファイルなので Read 必須。Read 省略すると `File has not been read yet` で失敗する）。12 件を JSONL 形式で書き出す
